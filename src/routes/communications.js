...
router.use(rateLimit({ windowMs: 15 * 60 * 1000, max: 12, standardHeaders: true, legacyHeaders: false, message: { error: 'Too many submissions. Please try again later.' } }));
...
const success = (res, referenceId, message) => res.json({ ok: true, reference: referenceId, message });
...
async function persistAndNotify({ table, prefix, data, recipient, subject, customerSubject, customerHtml, internalHtml }) {
  ...
  try {
    const internalSubject = subject.includes('PENDING') ? subject.replace('PENDING', ref) : `${subject} — ${ref}`;
    await sendMail({ to: recipient, subject: internalSubject, html: `${internalHtml}<p style="margin-top:20px"><strong>Reference:</strong> ${esc(ref)}</p>`, replyTo: data.email });
...
router.post('/quote', async (req, res) => {
  ...
  const data = { name: clean(body.name, 120), email: clean(body.email, 255).toLowerCase(), phone: clean(body.phone, 50), company: clean(body.company, 160), industry: clean(body.industry, 80), service: clean(body.service, 100), quantity: clean(body.quantity, 120), timeline: clean(body.timeline, 80), details: clean(body.details, 5000), status: 'NEW', notification_status: 'pending', mail_error: null };
  ...
  try {
    const ref = await persistAndNotify({ table: 'quote_requests', prefix: 'Q', data, recipient: config.mail.recipients.quotes, subject: 'New Quote Request — PENDING', customerSubject: 'Quote Request Received — Share Hubs Engineering', customerHtml: customerEmail('Quote request received', data.name, 'quote'), internalHtml: internalEmail('New Quote Request', data) });
...
router.post('/consultation', async (req, res) => {
  const body = req.body || {};
  if (clean(body._website, 100)) return success(res, '', 'Your request has been received.');
  const data = { name: clean(body.name, 120), email: clean(body.email, 255).toLowerCase(), phone: clean(body.phone, 50), company: clean(body.company, 160), topic: clean(body.topic, 120), preference: clean(body.preference, 80), message: clean(body.message, 5000), status: 'NEW', notification_status: 'pending', mail_error: null };
...
router.post('/contact', async (req, res) => {
  ...
  try { const ref = await persistAndNotify({ table: 'contact_requests', prefix: 'E', data, recipient: config.mail.recipients.info, subject: `New Enquiry — ${data.subject}`, customerSubject: 'We Received Your Message — Share Hubs Engineering', customerHtml: customerEmail('Message received', data.name, 'contact'), internalHtml: internalEmail('New Contact Enquiry', data) }); return success(res, ref, `Your message has been received. Reference: ${ref}`); }
...
router.post('/newsletter', async (req, res) => {
  ...
  try { await db.execute('INSERT INTO newsletter_subscribers (email, source) VALUES (?, ?) ON DUPLICATE KEY UPDATE subscribed_at = CURRENT_TIMESTAMP', [email, 'website']); await sendMail({ to: email, subject: 'You’re subscribed — Share Hubs Engineering', html: customerEmail('You’re subscribed', '', 'newsletter'), replyTo: config.mail.recipients.info }); return success(res, '', 'You’re subscribed to Share Hubs Engineering updates.'); }
...
function customerEmail(title, name, type) { const intro = name ? `Hello ${esc(name)},` : 'Hello,'; const body = type === 'quote' ? 'We have received your quote request and our team will review the information you provided. If we need additional details, we will contact you.' : type === 'consultation' ? 'We have received your consultation request. Our team will review your preferred contact method and requirements before getting in touch.' : type === 'newsletter' ? 'Thanks for subscribing. You’ll receive occasional updates about our projects, capabilities, and engineering insights.' : 'We have received your message and will review it as soon as possible.'; return layout(`<h1>${esc(title)}</h1><p>${intro}</p><p>${body}</p><p>Regards,<br><strong>Share Hubs Engineering</strong><br>Precision Manufacturing &amp; Engineering Solutions</p>`); }
function internalEmail(title, data) { const rows = Object.entries(data).filter(([k, v]) => v && !['mail_error', 'notification_status'].includes(k)).map(([k, v]) => `<tr><td style="padding:8px;font-weight:700;text-transform:capitalize">${esc(k.replaceAll('_', ' '))}</td><td style="padding:8px">${esc(v)}</td></tr>`).join(''); return layout(`<h1>${esc(title)}</h1><table style="width:100%;border-collapse:collapse">${rows}</table>`); }
