...
function setSessionCookie(res, token) {
  ...
  res.cookie('sh_admin_session', token, {
    httpOnly: true,
...
function clearSessionCookie(res) {
  res.clearCookie('sh_admin_session', { httpOnly: true, secure: config.env === 'production', sameSite: 'lax', path: '/admin' });
}
...
async function createSession(userId) {
  ...
  await db.query(
    'INSERT INTO admin_sessions (token_hash, user_id, expires_at) VALUES (?, ?, DATE_ADD(NOW(), INTERVAL ? DAY))',
...
async function requireAdmin(req, res, next) {
  try {
    ...
    const [rows] = await db.query(
      `SELECT u.id, u.name, u.email, u.role, u.verified, u.active
       FROM admin_sessions s
       JOIN admin_users u ON u.id = s.user_id
       WHERE s.token_hash = ? AND s.expires_at > NOW() AND u.active = 1 AND u.verified = 1
       LIMIT 1`,
      [hashToken(rawToken)]
    );
...
router.get('/register', (req, res) => {
  res.render('admin/register', { title: 'Staff Account — Share Hubs Engineering', error: req.query.error || null });
});
...
router.post('/register', authLimiter, [
  ...
  if (!isCompanyEmail(email)) {
    return res.redirect(`/admin/register?error=${encodeURIComponent('Staff accounts must use a @sharehubsengineering.com email address.')}`);
  }
  ...
  try {
    ...
    if (existing.length) {
      ...
      await db.query(
        `UPDATE admin_users SET name = ?, password_hash = ?, verification_token_hash = ?, verification_expires_at = DATE_ADD(NOW(), INTERVAL ? HOUR), active = 1 WHERE id = ?`,
        [name, passwordHash, tokenHash, VERIFICATION_TTL_HOURS, existing[0].id]
      );
    } else {
      await db.query(
        `INSERT INTO admin_users (name, email, password_hash, role, verification_token_hash, verification_expires_at) VALUES (?, ?, ?, 'STAFF', ?, DATE_ADD(NOW(), INTERVAL ? HOUR))`,
        [name, email, passwordHash, tokenHash, VERIFICATION_TTL_HOURS]
      );
    }
...
router.get('/verify/:token', async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      `UPDATE admin_users SET verified = 1, verification_token_hash = NULL, verification_expires_at = NULL WHERE verification_token_hash = ? AND verification_expires_at > NOW()`,
      [tokenHash]
    );
  ...
  try {
    const [[counts]] = await db.query(`
      SELECT
        ...
        (SELECT COUNT(*) FROM consultation_requests) AS total,
        (SELECT COUNT(*) FROM contacts WHERE created_at >= CURDATE()) +
        (SELECT COUNT(*) FROM quote_requests WHERE created_at >= CURDATE()) +
        (SELECT COUNT(*) FROM consultation_requests WHERE created_at >= CURDATE()) AS today,
        (SELECT COUNT(*) FROM admin_enquiry_status WHERE status = 'NEW') AS tracked_new,
        (SELECT COUNT(*) FROM admin_enquiry_status WHERE status = 'CONTACTED') AS contacted,
        (SELECT COUNT(*) FROM admin_enquiry_status WHERE status = 'WON') AS won
    `);

    const [enquiries] = await db.query(`
      SELECT * FROM (
        SELECT 'CONTACT' AS type, reference_id, name, email, phone, subject, NULL AS service, NULL AS company, created_at FROM contacts
        UNION ALL
        SELECT 'QUOTE' AS type, reference_id, name, email, phone, CONCAT('Quote request — ', service) AS subject, service, company, created_at FROM quote_requests
        UNION ALL
        SELECT 'CONSULTATION' AS type, reference_id, name, email, phone, CONCAT('Consultation — ', topic) AS subject, NULL AS service, company, created_at FROM consultation_requests
      ) enquiries
    ...
    if (references.length) {
      ...
      [statuses] = await db.query(`SELECT reference_id, status, assigned_to, internal_note FROM admin_enquiry_status WHERE reference_id IN (${placeholders})`, references);
    }
    ...
    return res.render('admin/dashboard', { title: 'Admin Dashboard — Share Hubs Engineering', admin: req.admin, counts: counts || {}, enquiries: rows, statuses: ALLOWED_STATUSES });
  } catch (error) {
  ...
  try {
    const [all] = await db.query(`
      SELECT * FROM (
        SELECT 'CONTACT' AS type, reference_id, name, email, phone, subject, NULL AS service, NULL AS company, created_at FROM contacts
        UNION ALL
        SELECT 'QUOTE', reference_id, name, email, phone, CONCAT('Quote request — ', service), service, company, created_at FROM quote_requests
        UNION ALL
        SELECT 'CONSULTATION', reference_id, name, email, phone, CONCAT('Consultation — ', topic), NULL, company, created_at FROM consultation_requests
      ) enquiries ORDER BY created_at DESC
    `);
    ...
    if (references.length) {
      const placeholders = references.map(() => '?').join(',');
      [statusRows] = await db.query(`SELECT reference_id, status, assigned_to FROM admin_enquiry_status WHERE reference_id IN (${placeholders})`, references);
    }
...
router.get('/api/overview', requireAdmin, async (req, res) => {
  try {
    const [[stats]] = await db.query(`
      SELECT
        (SELECT COUNT(*) FROM contacts) + (SELECT COUNT(*) FROM quote_requests) + (SELECT COUNT(*) FROM consultation_requests) AS total,
        (SELECT COUNT(*) FROM contacts WHERE created_at >= CURDATE()) + (SELECT COUNT(*) FROM quote_requests WHERE created_at >= CURDATE()) + (SELECT COUNT(*) FROM consultation_requests WHERE created_at >= CURDATE()) AS today,
        (SELECT COUNT(*) FROM admin_enquiry_status WHERE status = 'NEW') AS new_count,
        (SELECT COUNT(*) FROM admin_enquiry_status WHERE status = 'CONTACTED') AS contacted,
        (SELECT COUNT(*) FROM admin_enquiry_status WHERE status = 'WON') AS won
    `);
